...
void boost_waiting_processes(void)
{
    int boost_interval = 200; // set interval for when to boost process
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
        ...
        {
            if ((ticks - p->last_boost_tick) >= boost_interval) // if waited longer than interval
            {
                p->priority = 0;            // increase priority
                p->last_boost_tick = ticks; // reset boost timer
            }
        }
        release(&p->lock);
    }
...
void mlfq_scheduler(void)
{
    ...
    for (int i = 0; i < indexHigh; i++)
    {
        ...
        if (p->state == RUNNABLE)
        {
            ...
            c->proc = p;
            swtch(&c->context, &p->context);
            if (p->time_ticks > highTimeSlice) // check if time slot used up
            {
                p->priority = 1;   // demote
                p->time_ticks = 0; // reset time ticks
            }
            p->last_boost_tick = ticks; // set last boost to ticks
