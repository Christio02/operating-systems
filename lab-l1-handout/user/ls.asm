
user/_ls:     file format elf64-littleriscv


Disassembly of section .text:

0000000000000000 <fmtname>:
#include "user/user.h"
#include "kernel/fs.h"

char*
fmtname(char *path)
{
       0:	7139                	addi	sp,sp,-64
       2:	fc06                	sd	ra,56(sp)
       4:	f822                	sd	s0,48(sp)
       6:	f426                	sd	s1,40(sp)
       8:	0080                	addi	s0,sp,64
       a:	fca43423          	sd	a0,-56(s0)
  static char buf[DIRSIZ+1];
  char *p;

  // Find first character after last slash.
  for(p=path+strlen(path); p >= path && *p != '/'; p--)
       e:	fc843503          	ld	a0,-56(s0)
      12:	00000097          	auipc	ra,0x0
      16:	45a080e7          	jalr	1114(ra) # 46c <strlen>
      1a:	87aa                	mv	a5,a0
      1c:	2781                	sext.w	a5,a5
      1e:	1782                	slli	a5,a5,0x20
      20:	9381                	srli	a5,a5,0x20
      22:	fc843703          	ld	a4,-56(s0)
      26:	97ba                	add	a5,a5,a4
      28:	fcf43c23          	sd	a5,-40(s0)
      2c:	a031                	j	38 <fmtname+0x38>
      2e:	fd843783          	ld	a5,-40(s0)
      32:	17fd                	addi	a5,a5,-1
      34:	fcf43c23          	sd	a5,-40(s0)
      38:	fd843703          	ld	a4,-40(s0)
      3c:	fc843783          	ld	a5,-56(s0)
      40:	00f76b63          	bltu	a4,a5,56 <fmtname+0x56>
      44:	fd843783          	ld	a5,-40(s0)
      48:	0007c783          	lbu	a5,0(a5)
      4c:	873e                	mv	a4,a5
      4e:	02f00793          	li	a5,47
      52:	fcf71ee3          	bne	a4,a5,2e <fmtname+0x2e>
    ;
  p++;
      56:	fd843783          	ld	a5,-40(s0)
      5a:	0785                	addi	a5,a5,1
      5c:	fcf43c23          	sd	a5,-40(s0)

  // Return blank-padded name.
  if(strlen(p) >= DIRSIZ)
      60:	fd843503          	ld	a0,-40(s0)
      64:	00000097          	auipc	ra,0x0
      68:	408080e7          	jalr	1032(ra) # 46c <strlen>
      6c:	87aa                	mv	a5,a0
      6e:	2781                	sext.w	a5,a5
      70:	873e                	mv	a4,a5
      72:	47b5                	li	a5,13
      74:	00e7f563          	bgeu	a5,a4,7e <fmtname+0x7e>
    return p;
      78:	fd843783          	ld	a5,-40(s0)
      7c:	a8b5                	j	f8 <fmtname+0xf8>
  memmove(buf, p, strlen(p));
      7e:	fd843503          	ld	a0,-40(s0)
      82:	00000097          	auipc	ra,0x0
      86:	3ea080e7          	jalr	1002(ra) # 46c <strlen>
      8a:	87aa                	mv	a5,a0
      8c:	2781                	sext.w	a5,a5
      8e:	2781                	sext.w	a5,a5
      90:	863e                	mv	a2,a5
      92:	fd843583          	ld	a1,-40(s0)
      96:	00002517          	auipc	a0,0x2
      9a:	34a50513          	addi	a0,a0,842 # 23e0 <buf.0>
      9e:	00000097          	auipc	ra,0x0
      a2:	62a080e7          	jalr	1578(ra) # 6c8 <memmove>
  memset(buf+strlen(p), ' ', DIRSIZ-strlen(p));
      a6:	fd843503          	ld	a0,-40(s0)
      aa:	00000097          	auipc	ra,0x0
      ae:	3c2080e7          	jalr	962(ra) # 46c <strlen>
      b2:	87aa                	mv	a5,a0
      b4:	2781                	sext.w	a5,a5
      b6:	02079713          	slli	a4,a5,0x20
      ba:	9301                	srli	a4,a4,0x20
      bc:	00002797          	auipc	a5,0x2
      c0:	32478793          	addi	a5,a5,804 # 23e0 <buf.0>
      c4:	00f704b3          	add	s1,a4,a5
      c8:	fd843503          	ld	a0,-40(s0)
      cc:	00000097          	auipc	ra,0x0
      d0:	3a0080e7          	jalr	928(ra) # 46c <strlen>
      d4:	87aa                	mv	a5,a0
      d6:	2781                	sext.w	a5,a5
      d8:	4739                	li	a4,14
      da:	40f707bb          	subw	a5,a4,a5
      de:	2781                	sext.w	a5,a5
      e0:	863e                	mv	a2,a5
      e2:	02000593          	li	a1,32
      e6:	8526                	mv	a0,s1
      e8:	00000097          	auipc	ra,0x0
      ec:	3ba080e7          	jalr	954(ra) # 4a2 <memset>
  return buf;
      f0:	00002797          	auipc	a5,0x2
      f4:	2f078793          	addi	a5,a5,752 # 23e0 <buf.0>
}
      f8:	853e                	mv	a0,a5
      fa:	70e2                	ld	ra,56(sp)
      fc:	7442                	ld	s0,48(sp)
      fe:	74a2                	ld	s1,40(sp)
     100:	6121                	addi	sp,sp,64
     102:	8082                	ret

0000000000000104 <ls>:

void
ls(char *path)
{
     104:	da010113          	addi	sp,sp,-608
     108:	24113c23          	sd	ra,600(sp)
     10c:	24813823          	sd	s0,592(sp)
     110:	1480                	addi	s0,sp,608
     112:	daa43423          	sd	a0,-600(s0)
  char buf[512], *p;
  int fd;
  struct dirent de;
  struct stat st;

  if((fd = open(path, 0)) < 0){
     116:	4581                	li	a1,0
     118:	da843503          	ld	a0,-600(s0)
     11c:	00000097          	auipc	ra,0x0
     120:	772080e7          	jalr	1906(ra) # 88e <open>
     124:	87aa                	mv	a5,a0
     126:	fef42623          	sw	a5,-20(s0)
     12a:	fec42783          	lw	a5,-20(s0)
     12e:	2781                	sext.w	a5,a5
     130:	0007de63          	bgez	a5,14c <ls+0x48>
    fprintf(2, "ls: cannot open %s\n", path);
     134:	da843603          	ld	a2,-600(s0)
     138:	00001597          	auipc	a1,0x1
     13c:	f8858593          	addi	a1,a1,-120 # 10c0 <malloc+0x140>
     140:	4509                	li	a0,2
     142:	00001097          	auipc	ra,0x1
     146:	bf4080e7          	jalr	-1036(ra) # d36 <fprintf>
    return;
     14a:	a2e9                	j	314 <ls+0x210>
  }

  if(fstat(fd, &st) < 0){
     14c:	db840713          	addi	a4,s0,-584
     150:	fec42783          	lw	a5,-20(s0)
     154:	85ba                	mv	a1,a4
     156:	853e                	mv	a0,a5
     158:	00000097          	auipc	ra,0x0
     15c:	74e080e7          	jalr	1870(ra) # 8a6 <fstat>
     160:	87aa                	mv	a5,a0
     162:	0207d563          	bgez	a5,18c <ls+0x88>
    fprintf(2, "ls: cannot stat %s\n", path);
     166:	da843603          	ld	a2,-600(s0)
     16a:	00001597          	auipc	a1,0x1
     16e:	f6e58593          	addi	a1,a1,-146 # 10d8 <malloc+0x158>
     172:	4509                	li	a0,2
     174:	00001097          	auipc	ra,0x1
     178:	bc2080e7          	jalr	-1086(ra) # d36 <fprintf>
    close(fd);
     17c:	fec42783          	lw	a5,-20(s0)
     180:	853e                	mv	a0,a5
     182:	00000097          	auipc	ra,0x0
     186:	6f4080e7          	jalr	1780(ra) # 876 <close>
    return;
     18a:	a269                	j	314 <ls+0x210>
  }

  switch(st.type){
     18c:	dc041783          	lh	a5,-576(s0)
     190:	873e                	mv	a4,a5
     192:	86ba                	mv	a3,a4
     194:	4785                	li	a5,1
     196:	04f68563          	beq	a3,a5,1e0 <ls+0xdc>
     19a:	87ba                	mv	a5,a4
     19c:	16f05563          	blez	a5,306 <ls+0x202>
     1a0:	0007079b          	sext.w	a5,a4
     1a4:	37f9                	addiw	a5,a5,-2
     1a6:	2781                	sext.w	a5,a5
     1a8:	873e                	mv	a4,a5
     1aa:	4785                	li	a5,1
     1ac:	14e7ed63          	bltu	a5,a4,306 <ls+0x202>
  case T_DEVICE:
  case T_FILE:
    printf("%s %d %d %l\n", fmtname(path), st.type, st.ino, st.size);
     1b0:	da843503          	ld	a0,-600(s0)
     1b4:	00000097          	auipc	ra,0x0
     1b8:	e4c080e7          	jalr	-436(ra) # 0 <fmtname>
     1bc:	85aa                	mv	a1,a0
     1be:	dc041783          	lh	a5,-576(s0)
     1c2:	863e                	mv	a2,a5
     1c4:	dbc42783          	lw	a5,-580(s0)
     1c8:	dc843703          	ld	a4,-568(s0)
     1cc:	86be                	mv	a3,a5
     1ce:	00001517          	auipc	a0,0x1
     1d2:	f2250513          	addi	a0,a0,-222 # 10f0 <malloc+0x170>
     1d6:	00001097          	auipc	ra,0x1
     1da:	bb8080e7          	jalr	-1096(ra) # d8e <printf>
    break;
     1de:	a225                	j	306 <ls+0x202>

  case T_DIR:
    if(strlen(path) + 1 + DIRSIZ + 1 > sizeof buf){
     1e0:	da843503          	ld	a0,-600(s0)
     1e4:	00000097          	auipc	ra,0x0
     1e8:	288080e7          	jalr	648(ra) # 46c <strlen>
     1ec:	87aa                	mv	a5,a0
     1ee:	2781                	sext.w	a5,a5
     1f0:	27c1                	addiw	a5,a5,16
     1f2:	2781                	sext.w	a5,a5
     1f4:	873e                	mv	a4,a5
     1f6:	20000793          	li	a5,512
     1fa:	00e7fb63          	bgeu	a5,a4,210 <ls+0x10c>
      printf("ls: path too long\n");
     1fe:	00001517          	auipc	a0,0x1
     202:	f0250513          	addi	a0,a0,-254 # 1100 <malloc+0x180>
     206:	00001097          	auipc	ra,0x1
     20a:	b88080e7          	jalr	-1144(ra) # d8e <printf>
      break;
     20e:	a8e5                	j	306 <ls+0x202>
    }
    strcpy(buf, path);
     210:	de040793          	addi	a5,s0,-544
     214:	da843583          	ld	a1,-600(s0)
     218:	853e                	mv	a0,a5
     21a:	00000097          	auipc	ra,0x0
     21e:	1a2080e7          	jalr	418(ra) # 3bc <strcpy>
    p = buf+strlen(buf);
     222:	de040793          	addi	a5,s0,-544
     226:	853e                	mv	a0,a5
     228:	00000097          	auipc	ra,0x0
     22c:	244080e7          	jalr	580(ra) # 46c <strlen>
     230:	87aa                	mv	a5,a0
     232:	2781                	sext.w	a5,a5
     234:	1782                	slli	a5,a5,0x20
     236:	9381                	srli	a5,a5,0x20
     238:	de040713          	addi	a4,s0,-544
     23c:	97ba                	add	a5,a5,a4
     23e:	fef43023          	sd	a5,-32(s0)
    *p++ = '/';
     242:	fe043783          	ld	a5,-32(s0)
     246:	00178713          	addi	a4,a5,1
     24a:	fee43023          	sd	a4,-32(s0)
     24e:	02f00713          	li	a4,47
     252:	00e78023          	sb	a4,0(a5)
    while(read(fd, &de, sizeof(de)) == sizeof(de)){
     256:	a079                	j	2e4 <ls+0x1e0>
      if(de.inum == 0)
     258:	dd045783          	lhu	a5,-560(s0)
     25c:	c3d9                	beqz	a5,2e2 <ls+0x1de>
        continue;
      memmove(p, de.name, DIRSIZ);
     25e:	dd040793          	addi	a5,s0,-560
     262:	0789                	addi	a5,a5,2
     264:	4639                	li	a2,14
     266:	85be                	mv	a1,a5
     268:	fe043503          	ld	a0,-32(s0)
     26c:	00000097          	auipc	ra,0x0
     270:	45c080e7          	jalr	1116(ra) # 6c8 <memmove>
      p[DIRSIZ] = 0;
     274:	fe043783          	ld	a5,-32(s0)
     278:	07b9                	addi	a5,a5,14
     27a:	00078023          	sb	zero,0(a5)
      if(stat(buf, &st) < 0){
     27e:	db840713          	addi	a4,s0,-584
     282:	de040793          	addi	a5,s0,-544
     286:	85ba                	mv	a1,a4
     288:	853e                	mv	a0,a5
     28a:	00000097          	auipc	ra,0x0
     28e:	364080e7          	jalr	868(ra) # 5ee <stat>
     292:	87aa                	mv	a5,a0
     294:	0007de63          	bgez	a5,2b0 <ls+0x1ac>
        printf("ls: cannot stat %s\n", buf);
     298:	de040793          	addi	a5,s0,-544
     29c:	85be                	mv	a1,a5
     29e:	00001517          	auipc	a0,0x1
     2a2:	e3a50513          	addi	a0,a0,-454 # 10d8 <malloc+0x158>
     2a6:	00001097          	auipc	ra,0x1
     2aa:	ae8080e7          	jalr	-1304(ra) # d8e <printf>
        continue;
     2ae:	a81d                	j	2e4 <ls+0x1e0>
      }
      printf("%s %d %d %d\n", fmtname(buf), st.type, st.ino, st.size);
     2b0:	de040793          	addi	a5,s0,-544
     2b4:	853e                	mv	a0,a5
     2b6:	00000097          	auipc	ra,0x0
     2ba:	d4a080e7          	jalr	-694(ra) # 0 <fmtname>
     2be:	85aa                	mv	a1,a0
     2c0:	dc041783          	lh	a5,-576(s0)
     2c4:	863e                	mv	a2,a5
     2c6:	dbc42783          	lw	a5,-580(s0)
     2ca:	dc843703          	ld	a4,-568(s0)
     2ce:	86be                	mv	a3,a5
     2d0:	00001517          	auipc	a0,0x1
     2d4:	e4850513          	addi	a0,a0,-440 # 1118 <malloc+0x198>
     2d8:	00001097          	auipc	ra,0x1
     2dc:	ab6080e7          	jalr	-1354(ra) # d8e <printf>
     2e0:	a011                	j	2e4 <ls+0x1e0>
        continue;
     2e2:	0001                	nop
    while(read(fd, &de, sizeof(de)) == sizeof(de)){
     2e4:	dd040713          	addi	a4,s0,-560
     2e8:	fec42783          	lw	a5,-20(s0)
     2ec:	4641                	li	a2,16
     2ee:	85ba                	mv	a1,a4
     2f0:	853e                	mv	a0,a5
     2f2:	00000097          	auipc	ra,0x0
     2f6:	574080e7          	jalr	1396(ra) # 866 <read>
     2fa:	87aa                	mv	a5,a0
     2fc:	873e                	mv	a4,a5
     2fe:	47c1                	li	a5,16
     300:	f4f70ce3          	beq	a4,a5,258 <ls+0x154>
    }
    break;
     304:	0001                	nop
  }
  close(fd);
     306:	fec42783          	lw	a5,-20(s0)
     30a:	853e                	mv	a0,a5
     30c:	00000097          	auipc	ra,0x0
     310:	56a080e7          	jalr	1386(ra) # 876 <close>
}
     314:	25813083          	ld	ra,600(sp)
     318:	25013403          	ld	s0,592(sp)
     31c:	26010113          	addi	sp,sp,608
     320:	8082                	ret

0000000000000322 <main>:

int
main(int argc, char *argv[])
{
     322:	7179                	addi	sp,sp,-48
     324:	f406                	sd	ra,40(sp)
     326:	f022                	sd	s0,32(sp)
     328:	1800                	addi	s0,sp,48
     32a:	87aa                	mv	a5,a0
     32c:	fcb43823          	sd	a1,-48(s0)
     330:	fcf42e23          	sw	a5,-36(s0)
  int i;

  if(argc < 2){
     334:	fdc42783          	lw	a5,-36(s0)
     338:	0007871b          	sext.w	a4,a5
     33c:	4785                	li	a5,1
     33e:	00e7cf63          	blt	a5,a4,35c <main+0x3a>
    ls(".");
     342:	00001517          	auipc	a0,0x1
     346:	de650513          	addi	a0,a0,-538 # 1128 <malloc+0x1a8>
     34a:	00000097          	auipc	ra,0x0
     34e:	dba080e7          	jalr	-582(ra) # 104 <ls>
    exit(0);
     352:	4501                	li	a0,0
     354:	00000097          	auipc	ra,0x0
     358:	4fa080e7          	jalr	1274(ra) # 84e <exit>
  }
  for(i=1; i<argc; i++)
     35c:	4785                	li	a5,1
     35e:	fef42623          	sw	a5,-20(s0)
     362:	a015                	j	386 <main+0x64>
    ls(argv[i]);
     364:	fec42783          	lw	a5,-20(s0)
     368:	078e                	slli	a5,a5,0x3
     36a:	fd043703          	ld	a4,-48(s0)
     36e:	97ba                	add	a5,a5,a4
     370:	639c                	ld	a5,0(a5)
     372:	853e                	mv	a0,a5
     374:	00000097          	auipc	ra,0x0
     378:	d90080e7          	jalr	-624(ra) # 104 <ls>
  for(i=1; i<argc; i++)
     37c:	fec42783          	lw	a5,-20(s0)
     380:	2785                	addiw	a5,a5,1
     382:	fef42623          	sw	a5,-20(s0)
     386:	fec42783          	lw	a5,-20(s0)
     38a:	873e                	mv	a4,a5
     38c:	fdc42783          	lw	a5,-36(s0)
     390:	2701                	sext.w	a4,a4
     392:	2781                	sext.w	a5,a5
     394:	fcf748e3          	blt	a4,a5,364 <main+0x42>
  exit(0);
     398:	4501                	li	a0,0
     39a:	00000097          	auipc	ra,0x0
     39e:	4b4080e7          	jalr	1204(ra) # 84e <exit>

00000000000003a2 <_main>:
//
// wrapper so that it's OK if main() does not call exit().
//
void
_main()
{
     3a2:	1141                	addi	sp,sp,-16
     3a4:	e406                	sd	ra,8(sp)
     3a6:	e022                	sd	s0,0(sp)
     3a8:	0800                	addi	s0,sp,16
  extern int main();
  main();
     3aa:	00000097          	auipc	ra,0x0
     3ae:	f78080e7          	jalr	-136(ra) # 322 <main>
  exit(0);
     3b2:	4501                	li	a0,0
     3b4:	00000097          	auipc	ra,0x0
     3b8:	49a080e7          	jalr	1178(ra) # 84e <exit>

00000000000003bc <strcpy>:
}

char*
strcpy(char *s, const char *t)
{
     3bc:	7179                	addi	sp,sp,-48
     3be:	f422                	sd	s0,40(sp)
     3c0:	1800                	addi	s0,sp,48
     3c2:	fca43c23          	sd	a0,-40(s0)
     3c6:	fcb43823          	sd	a1,-48(s0)
  char *os;

  os = s;
     3ca:	fd843783          	ld	a5,-40(s0)
     3ce:	fef43423          	sd	a5,-24(s0)
  while((*s++ = *t++) != 0)
     3d2:	0001                	nop
     3d4:	fd043703          	ld	a4,-48(s0)
     3d8:	00170793          	addi	a5,a4,1
     3dc:	fcf43823          	sd	a5,-48(s0)
     3e0:	fd843783          	ld	a5,-40(s0)
     3e4:	00178693          	addi	a3,a5,1
     3e8:	fcd43c23          	sd	a3,-40(s0)
     3ec:	00074703          	lbu	a4,0(a4)
     3f0:	00e78023          	sb	a4,0(a5)
     3f4:	0007c783          	lbu	a5,0(a5)
     3f8:	fff1                	bnez	a5,3d4 <strcpy+0x18>
    ;
  return os;
     3fa:	fe843783          	ld	a5,-24(s0)
}
     3fe:	853e                	mv	a0,a5
     400:	7422                	ld	s0,40(sp)
     402:	6145                	addi	sp,sp,48
     404:	8082                	ret

0000000000000406 <strcmp>:

int
strcmp(const char *p, const char *q)
{
     406:	1101                	addi	sp,sp,-32
     408:	ec22                	sd	s0,24(sp)
     40a:	1000                	addi	s0,sp,32
     40c:	fea43423          	sd	a0,-24(s0)
     410:	feb43023          	sd	a1,-32(s0)
  while(*p && *p == *q)
     414:	a819                	j	42a <strcmp+0x24>
    p++, q++;
     416:	fe843783          	ld	a5,-24(s0)
     41a:	0785                	addi	a5,a5,1
     41c:	fef43423          	sd	a5,-24(s0)
     420:	fe043783          	ld	a5,-32(s0)
     424:	0785                	addi	a5,a5,1
     426:	fef43023          	sd	a5,-32(s0)
  while(*p && *p == *q)
     42a:	fe843783          	ld	a5,-24(s0)
     42e:	0007c783          	lbu	a5,0(a5)
     432:	cb99                	beqz	a5,448 <strcmp+0x42>
     434:	fe843783          	ld	a5,-24(s0)
     438:	0007c703          	lbu	a4,0(a5)
     43c:	fe043783          	ld	a5,-32(s0)
     440:	0007c783          	lbu	a5,0(a5)
     444:	fcf709e3          	beq	a4,a5,416 <strcmp+0x10>
  return (uchar)*p - (uchar)*q;
     448:	fe843783          	ld	a5,-24(s0)
     44c:	0007c783          	lbu	a5,0(a5)
     450:	0007871b          	sext.w	a4,a5
     454:	fe043783          	ld	a5,-32(s0)
     458:	0007c783          	lbu	a5,0(a5)
     45c:	2781                	sext.w	a5,a5
     45e:	40f707bb          	subw	a5,a4,a5
     462:	2781                	sext.w	a5,a5
}
     464:	853e                	mv	a0,a5
     466:	6462                	ld	s0,24(sp)
     468:	6105                	addi	sp,sp,32
     46a:	8082                	ret

000000000000046c <strlen>:

uint
strlen(const char *s)
{
     46c:	7179                	addi	sp,sp,-48
     46e:	f422                	sd	s0,40(sp)
     470:	1800                	addi	s0,sp,48
     472:	fca43c23          	sd	a0,-40(s0)
  int n;

  for(n = 0; s[n]; n++)
     476:	fe042623          	sw	zero,-20(s0)
     47a:	a031                	j	486 <strlen+0x1a>
     47c:	fec42783          	lw	a5,-20(s0)
     480:	2785                	addiw	a5,a5,1
     482:	fef42623          	sw	a5,-20(s0)
     486:	fec42783          	lw	a5,-20(s0)
     48a:	fd843703          	ld	a4,-40(s0)
     48e:	97ba                	add	a5,a5,a4
     490:	0007c783          	lbu	a5,0(a5)
     494:	f7e5                	bnez	a5,47c <strlen+0x10>
    ;
  return n;
     496:	fec42783          	lw	a5,-20(s0)
}
     49a:	853e                	mv	a0,a5
     49c:	7422                	ld	s0,40(sp)
     49e:	6145                	addi	sp,sp,48
     4a0:	8082                	ret

00000000000004a2 <memset>:

void*
memset(void *dst, int c, uint n)
{
     4a2:	7179                	addi	sp,sp,-48
     4a4:	f422                	sd	s0,40(sp)
     4a6:	1800                	addi	s0,sp,48
     4a8:	fca43c23          	sd	a0,-40(s0)
     4ac:	87ae                	mv	a5,a1
     4ae:	8732                	mv	a4,a2
     4b0:	fcf42a23          	sw	a5,-44(s0)
     4b4:	87ba                	mv	a5,a4
     4b6:	fcf42823          	sw	a5,-48(s0)
  char *cdst = (char *) dst;
     4ba:	fd843783          	ld	a5,-40(s0)
     4be:	fef43023          	sd	a5,-32(s0)
  int i;
  for(i = 0; i < n; i++){
     4c2:	fe042623          	sw	zero,-20(s0)
     4c6:	a00d                	j	4e8 <memset+0x46>
    cdst[i] = c;
     4c8:	fec42783          	lw	a5,-20(s0)
     4cc:	fe043703          	ld	a4,-32(s0)
     4d0:	97ba                	add	a5,a5,a4
     4d2:	fd442703          	lw	a4,-44(s0)
     4d6:	0ff77713          	zext.b	a4,a4
     4da:	00e78023          	sb	a4,0(a5)
  for(i = 0; i < n; i++){
     4de:	fec42783          	lw	a5,-20(s0)
     4e2:	2785                	addiw	a5,a5,1
     4e4:	fef42623          	sw	a5,-20(s0)
     4e8:	fec42703          	lw	a4,-20(s0)
     4ec:	fd042783          	lw	a5,-48(s0)
     4f0:	2781                	sext.w	a5,a5
     4f2:	fcf76be3          	bltu	a4,a5,4c8 <memset+0x26>
  }
  return dst;
     4f6:	fd843783          	ld	a5,-40(s0)
}
     4fa:	853e                	mv	a0,a5
     4fc:	7422                	ld	s0,40(sp)
     4fe:	6145                	addi	sp,sp,48
     500:	8082                	ret

0000000000000502 <strchr>:

char*
strchr(const char *s, char c)
{
     502:	1101                	addi	sp,sp,-32
     504:	ec22                	sd	s0,24(sp)
     506:	1000                	addi	s0,sp,32
     508:	fea43423          	sd	a0,-24(s0)
     50c:	87ae                	mv	a5,a1
     50e:	fef403a3          	sb	a5,-25(s0)
  for(; *s; s++)
     512:	a01d                	j	538 <strchr+0x36>
    if(*s == c)
     514:	fe843783          	ld	a5,-24(s0)
     518:	0007c703          	lbu	a4,0(a5)
     51c:	fe744783          	lbu	a5,-25(s0)
     520:	0ff7f793          	zext.b	a5,a5
     524:	00e79563          	bne	a5,a4,52e <strchr+0x2c>
      return (char*)s;
     528:	fe843783          	ld	a5,-24(s0)
     52c:	a821                	j	544 <strchr+0x42>
  for(; *s; s++)
     52e:	fe843783          	ld	a5,-24(s0)
     532:	0785                	addi	a5,a5,1
     534:	fef43423          	sd	a5,-24(s0)
     538:	fe843783          	ld	a5,-24(s0)
     53c:	0007c783          	lbu	a5,0(a5)
     540:	fbf1                	bnez	a5,514 <strchr+0x12>
  return 0;
     542:	4781                	li	a5,0
}
     544:	853e                	mv	a0,a5
     546:	6462                	ld	s0,24(sp)
     548:	6105                	addi	sp,sp,32
     54a:	8082                	ret

000000000000054c <gets>:

char*
gets(char *buf, int max)
{
     54c:	7179                	addi	sp,sp,-48
     54e:	f406                	sd	ra,40(sp)
     550:	f022                	sd	s0,32(sp)
     552:	1800                	addi	s0,sp,48
     554:	fca43c23          	sd	a0,-40(s0)
     558:	87ae                	mv	a5,a1
     55a:	fcf42a23          	sw	a5,-44(s0)
  int i, cc;
  char c;

  for(i=0; i+1 < max; ){
     55e:	fe042623          	sw	zero,-20(s0)
     562:	a8a1                	j	5ba <gets+0x6e>
    cc = read(0, &c, 1);
     564:	fe740793          	addi	a5,s0,-25
     568:	4605                	li	a2,1
     56a:	85be                	mv	a1,a5
     56c:	4501                	li	a0,0
     56e:	00000097          	auipc	ra,0x0
     572:	2f8080e7          	jalr	760(ra) # 866 <read>
     576:	87aa                	mv	a5,a0
     578:	fef42423          	sw	a5,-24(s0)
    if(cc < 1)
     57c:	fe842783          	lw	a5,-24(s0)
     580:	2781                	sext.w	a5,a5
     582:	04f05763          	blez	a5,5d0 <gets+0x84>
      break;
    buf[i++] = c;
     586:	fec42783          	lw	a5,-20(s0)
     58a:	0017871b          	addiw	a4,a5,1
     58e:	fee42623          	sw	a4,-20(s0)
     592:	873e                	mv	a4,a5
     594:	fd843783          	ld	a5,-40(s0)
     598:	97ba                	add	a5,a5,a4
     59a:	fe744703          	lbu	a4,-25(s0)
     59e:	00e78023          	sb	a4,0(a5)
    if(c == '\n' || c == '\r')
     5a2:	fe744783          	lbu	a5,-25(s0)
     5a6:	873e                	mv	a4,a5
     5a8:	47a9                	li	a5,10
     5aa:	02f70463          	beq	a4,a5,5d2 <gets+0x86>
     5ae:	fe744783          	lbu	a5,-25(s0)
     5b2:	873e                	mv	a4,a5
     5b4:	47b5                	li	a5,13
     5b6:	00f70e63          	beq	a4,a5,5d2 <gets+0x86>
  for(i=0; i+1 < max; ){
     5ba:	fec42783          	lw	a5,-20(s0)
     5be:	2785                	addiw	a5,a5,1
     5c0:	0007871b          	sext.w	a4,a5
     5c4:	fd442783          	lw	a5,-44(s0)
     5c8:	2781                	sext.w	a5,a5
     5ca:	f8f74de3          	blt	a4,a5,564 <gets+0x18>
     5ce:	a011                	j	5d2 <gets+0x86>
      break;
     5d0:	0001                	nop
      break;
  }
  buf[i] = '\0';
     5d2:	fec42783          	lw	a5,-20(s0)
     5d6:	fd843703          	ld	a4,-40(s0)
     5da:	97ba                	add	a5,a5,a4
     5dc:	00078023          	sb	zero,0(a5)
  return buf;
     5e0:	fd843783          	ld	a5,-40(s0)
}
     5e4:	853e                	mv	a0,a5
     5e6:	70a2                	ld	ra,40(sp)
     5e8:	7402                	ld	s0,32(sp)
     5ea:	6145                	addi	sp,sp,48
     5ec:	8082                	ret

00000000000005ee <stat>:

int
stat(const char *n, struct stat *st)
{
     5ee:	7179                	addi	sp,sp,-48
     5f0:	f406                	sd	ra,40(sp)
     5f2:	f022                	sd	s0,32(sp)
     5f4:	1800                	addi	s0,sp,48
     5f6:	fca43c23          	sd	a0,-40(s0)
     5fa:	fcb43823          	sd	a1,-48(s0)
  int fd;
  int r;

  fd = open(n, O_RDONLY);
     5fe:	4581                	li	a1,0
     600:	fd843503          	ld	a0,-40(s0)
     604:	00000097          	auipc	ra,0x0
     608:	28a080e7          	jalr	650(ra) # 88e <open>
     60c:	87aa                	mv	a5,a0
     60e:	fef42623          	sw	a5,-20(s0)
  if(fd < 0)
     612:	fec42783          	lw	a5,-20(s0)
     616:	2781                	sext.w	a5,a5
     618:	0007d463          	bgez	a5,620 <stat+0x32>
    return -1;
     61c:	57fd                	li	a5,-1
     61e:	a035                	j	64a <stat+0x5c>
  r = fstat(fd, st);
     620:	fec42783          	lw	a5,-20(s0)
     624:	fd043583          	ld	a1,-48(s0)
     628:	853e                	mv	a0,a5
     62a:	00000097          	auipc	ra,0x0
     62e:	27c080e7          	jalr	636(ra) # 8a6 <fstat>
     632:	87aa                	mv	a5,a0
     634:	fef42423          	sw	a5,-24(s0)
  close(fd);
     638:	fec42783          	lw	a5,-20(s0)
     63c:	853e                	mv	a0,a5
     63e:	00000097          	auipc	ra,0x0
     642:	238080e7          	jalr	568(ra) # 876 <close>
  return r;
     646:	fe842783          	lw	a5,-24(s0)
}
     64a:	853e                	mv	a0,a5
     64c:	70a2                	ld	ra,40(sp)
     64e:	7402                	ld	s0,32(sp)
     650:	6145                	addi	sp,sp,48
     652:	8082                	ret

0000000000000654 <atoi>:

int
atoi(const char *s)
{
     654:	7179                	addi	sp,sp,-48
     656:	f422                	sd	s0,40(sp)
     658:	1800                	addi	s0,sp,48
     65a:	fca43c23          	sd	a0,-40(s0)
  int n;

  n = 0;
     65e:	fe042623          	sw	zero,-20(s0)
  while('0' <= *s && *s <= '9')
     662:	a81d                	j	698 <atoi+0x44>
    n = n*10 + *s++ - '0';
     664:	fec42783          	lw	a5,-20(s0)
     668:	873e                	mv	a4,a5
     66a:	87ba                	mv	a5,a4
     66c:	0027979b          	slliw	a5,a5,0x2
     670:	9fb9                	addw	a5,a5,a4
     672:	0017979b          	slliw	a5,a5,0x1
     676:	0007871b          	sext.w	a4,a5
     67a:	fd843783          	ld	a5,-40(s0)
     67e:	00178693          	addi	a3,a5,1
     682:	fcd43c23          	sd	a3,-40(s0)
     686:	0007c783          	lbu	a5,0(a5)
     68a:	2781                	sext.w	a5,a5
     68c:	9fb9                	addw	a5,a5,a4
     68e:	2781                	sext.w	a5,a5
     690:	fd07879b          	addiw	a5,a5,-48
     694:	fef42623          	sw	a5,-20(s0)
  while('0' <= *s && *s <= '9')
     698:	fd843783          	ld	a5,-40(s0)
     69c:	0007c783          	lbu	a5,0(a5)
     6a0:	873e                	mv	a4,a5
     6a2:	02f00793          	li	a5,47
     6a6:	00e7fb63          	bgeu	a5,a4,6bc <atoi+0x68>
     6aa:	fd843783          	ld	a5,-40(s0)
     6ae:	0007c783          	lbu	a5,0(a5)
     6b2:	873e                	mv	a4,a5
     6b4:	03900793          	li	a5,57
     6b8:	fae7f6e3          	bgeu	a5,a4,664 <atoi+0x10>
  return n;
     6bc:	fec42783          	lw	a5,-20(s0)
}
     6c0:	853e                	mv	a0,a5
     6c2:	7422                	ld	s0,40(sp)
     6c4:	6145                	addi	sp,sp,48
     6c6:	8082                	ret

00000000000006c8 <memmove>:

void*
memmove(void *vdst, const void *vsrc, int n)
{
     6c8:	7139                	addi	sp,sp,-64
     6ca:	fc22                	sd	s0,56(sp)
     6cc:	0080                	addi	s0,sp,64
     6ce:	fca43c23          	sd	a0,-40(s0)
     6d2:	fcb43823          	sd	a1,-48(s0)
     6d6:	87b2                	mv	a5,a2
     6d8:	fcf42623          	sw	a5,-52(s0)
  char *dst;
  const char *src;

  dst = vdst;
     6dc:	fd843783          	ld	a5,-40(s0)
     6e0:	fef43423          	sd	a5,-24(s0)
  src = vsrc;
     6e4:	fd043783          	ld	a5,-48(s0)
     6e8:	fef43023          	sd	a5,-32(s0)
  if (src > dst) {
     6ec:	fe043703          	ld	a4,-32(s0)
     6f0:	fe843783          	ld	a5,-24(s0)
     6f4:	02e7fc63          	bgeu	a5,a4,72c <memmove+0x64>
    while(n-- > 0)
     6f8:	a00d                	j	71a <memmove+0x52>
      *dst++ = *src++;
     6fa:	fe043703          	ld	a4,-32(s0)
     6fe:	00170793          	addi	a5,a4,1
     702:	fef43023          	sd	a5,-32(s0)
     706:	fe843783          	ld	a5,-24(s0)
     70a:	00178693          	addi	a3,a5,1
     70e:	fed43423          	sd	a3,-24(s0)
     712:	00074703          	lbu	a4,0(a4)
     716:	00e78023          	sb	a4,0(a5)
    while(n-- > 0)
     71a:	fcc42783          	lw	a5,-52(s0)
     71e:	fff7871b          	addiw	a4,a5,-1
     722:	fce42623          	sw	a4,-52(s0)
     726:	fcf04ae3          	bgtz	a5,6fa <memmove+0x32>
     72a:	a891                	j	77e <memmove+0xb6>
  } else {
    dst += n;
     72c:	fcc42783          	lw	a5,-52(s0)
     730:	fe843703          	ld	a4,-24(s0)
     734:	97ba                	add	a5,a5,a4
     736:	fef43423          	sd	a5,-24(s0)
    src += n;
     73a:	fcc42783          	lw	a5,-52(s0)
     73e:	fe043703          	ld	a4,-32(s0)
     742:	97ba                	add	a5,a5,a4
     744:	fef43023          	sd	a5,-32(s0)
    while(n-- > 0)
     748:	a01d                	j	76e <memmove+0xa6>
      *--dst = *--src;
     74a:	fe043783          	ld	a5,-32(s0)
     74e:	17fd                	addi	a5,a5,-1
     750:	fef43023          	sd	a5,-32(s0)
     754:	fe843783          	ld	a5,-24(s0)
     758:	17fd                	addi	a5,a5,-1
     75a:	fef43423          	sd	a5,-24(s0)
     75e:	fe043783          	ld	a5,-32(s0)
     762:	0007c703          	lbu	a4,0(a5)
     766:	fe843783          	ld	a5,-24(s0)
     76a:	00e78023          	sb	a4,0(a5)
    while(n-- > 0)
     76e:	fcc42783          	lw	a5,-52(s0)
     772:	fff7871b          	addiw	a4,a5,-1
     776:	fce42623          	sw	a4,-52(s0)
     77a:	fcf048e3          	bgtz	a5,74a <memmove+0x82>
  }
  return vdst;
     77e:	fd843783          	ld	a5,-40(s0)
}
     782:	853e                	mv	a0,a5
     784:	7462                	ld	s0,56(sp)
     786:	6121                	addi	sp,sp,64
     788:	8082                	ret

000000000000078a <memcmp>:

int
memcmp(const void *s1, const void *s2, uint n)
{
     78a:	7139                	addi	sp,sp,-64
     78c:	fc22                	sd	s0,56(sp)
     78e:	0080                	addi	s0,sp,64
     790:	fca43c23          	sd	a0,-40(s0)
     794:	fcb43823          	sd	a1,-48(s0)
     798:	87b2                	mv	a5,a2
     79a:	fcf42623          	sw	a5,-52(s0)
  const char *p1 = s1, *p2 = s2;
     79e:	fd843783          	ld	a5,-40(s0)
     7a2:	fef43423          	sd	a5,-24(s0)
     7a6:	fd043783          	ld	a5,-48(s0)
     7aa:	fef43023          	sd	a5,-32(s0)
  while (n-- > 0) {
     7ae:	a0a1                	j	7f6 <memcmp+0x6c>
    if (*p1 != *p2) {
     7b0:	fe843783          	ld	a5,-24(s0)
     7b4:	0007c703          	lbu	a4,0(a5)
     7b8:	fe043783          	ld	a5,-32(s0)
     7bc:	0007c783          	lbu	a5,0(a5)
     7c0:	02f70163          	beq	a4,a5,7e2 <memcmp+0x58>
      return *p1 - *p2;
     7c4:	fe843783          	ld	a5,-24(s0)
     7c8:	0007c783          	lbu	a5,0(a5)
     7cc:	0007871b          	sext.w	a4,a5
     7d0:	fe043783          	ld	a5,-32(s0)
     7d4:	0007c783          	lbu	a5,0(a5)
     7d8:	2781                	sext.w	a5,a5
     7da:	40f707bb          	subw	a5,a4,a5
     7de:	2781                	sext.w	a5,a5
     7e0:	a01d                	j	806 <memcmp+0x7c>
    }
    p1++;
     7e2:	fe843783          	ld	a5,-24(s0)
     7e6:	0785                	addi	a5,a5,1
     7e8:	fef43423          	sd	a5,-24(s0)
    p2++;
     7ec:	fe043783          	ld	a5,-32(s0)
     7f0:	0785                	addi	a5,a5,1
     7f2:	fef43023          	sd	a5,-32(s0)
  while (n-- > 0) {
     7f6:	fcc42783          	lw	a5,-52(s0)
     7fa:	fff7871b          	addiw	a4,a5,-1
     7fe:	fce42623          	sw	a4,-52(s0)
     802:	f7dd                	bnez	a5,7b0 <memcmp+0x26>
  }
  return 0;
     804:	4781                	li	a5,0
}
     806:	853e                	mv	a0,a5
     808:	7462                	ld	s0,56(sp)
     80a:	6121                	addi	sp,sp,64
     80c:	8082                	ret

000000000000080e <memcpy>:

void *
memcpy(void *dst, const void *src, uint n)
{
     80e:	7179                	addi	sp,sp,-48
     810:	f406                	sd	ra,40(sp)
     812:	f022                	sd	s0,32(sp)
     814:	1800                	addi	s0,sp,48
     816:	fea43423          	sd	a0,-24(s0)
     81a:	feb43023          	sd	a1,-32(s0)
     81e:	87b2                	mv	a5,a2
     820:	fcf42e23          	sw	a5,-36(s0)
  return memmove(dst, src, n);
     824:	fdc42783          	lw	a5,-36(s0)
     828:	863e                	mv	a2,a5
     82a:	fe043583          	ld	a1,-32(s0)
     82e:	fe843503          	ld	a0,-24(s0)
     832:	00000097          	auipc	ra,0x0
     836:	e96080e7          	jalr	-362(ra) # 6c8 <memmove>
     83a:	87aa                	mv	a5,a0
}
     83c:	853e                	mv	a0,a5
     83e:	70a2                	ld	ra,40(sp)
     840:	7402                	ld	s0,32(sp)
     842:	6145                	addi	sp,sp,48
     844:	8082                	ret

0000000000000846 <fork>:
# generated by usys.pl - do not edit
#include "kernel/syscall.h"
.global fork
fork:
 li a7, SYS_fork
     846:	4885                	li	a7,1
 ecall
     848:	00000073          	ecall
 ret
     84c:	8082                	ret

000000000000084e <exit>:
.global exit
exit:
 li a7, SYS_exit
     84e:	4889                	li	a7,2
 ecall
     850:	00000073          	ecall
 ret
     854:	8082                	ret

0000000000000856 <wait>:
.global wait
wait:
 li a7, SYS_wait
     856:	488d                	li	a7,3
 ecall
     858:	00000073          	ecall
 ret
     85c:	8082                	ret

000000000000085e <pipe>:
.global pipe
pipe:
 li a7, SYS_pipe
     85e:	4891                	li	a7,4
 ecall
     860:	00000073          	ecall
 ret
     864:	8082                	ret

0000000000000866 <read>:
.global read
read:
 li a7, SYS_read
     866:	4895                	li	a7,5
 ecall
     868:	00000073          	ecall
 ret
     86c:	8082                	ret

000000000000086e <write>:
.global write
write:
 li a7, SYS_write
     86e:	48c1                	li	a7,16
 ecall
     870:	00000073          	ecall
 ret
     874:	8082                	ret

0000000000000876 <close>:
.global close
close:
 li a7, SYS_close
     876:	48d5                	li	a7,21
 ecall
     878:	00000073          	ecall
 ret
     87c:	8082                	ret

000000000000087e <kill>:
.global kill
kill:
 li a7, SYS_kill
     87e:	4899                	li	a7,6
 ecall
     880:	00000073          	ecall
 ret
     884:	8082                	ret

0000000000000886 <exec>:
.global exec
exec:
 li a7, SYS_exec
     886:	489d                	li	a7,7
 ecall
     888:	00000073          	ecall
 ret
     88c:	8082                	ret

000000000000088e <open>:
.global open
open:
 li a7, SYS_open
     88e:	48bd                	li	a7,15
 ecall
     890:	00000073          	ecall
 ret
     894:	8082                	ret

0000000000000896 <mknod>:
.global mknod
mknod:
 li a7, SYS_mknod
     896:	48c5                	li	a7,17
 ecall
     898:	00000073          	ecall
 ret
     89c:	8082                	ret

000000000000089e <unlink>:
.global unlink
unlink:
 li a7, SYS_unlink
     89e:	48c9                	li	a7,18
 ecall
     8a0:	00000073          	ecall
 ret
     8a4:	8082                	ret

00000000000008a6 <fstat>:
.global fstat
fstat:
 li a7, SYS_fstat
     8a6:	48a1                	li	a7,8
 ecall
     8a8:	00000073          	ecall
 ret
     8ac:	8082                	ret

00000000000008ae <link>:
.global link
link:
 li a7, SYS_link
     8ae:	48cd                	li	a7,19
 ecall
     8b0:	00000073          	ecall
 ret
     8b4:	8082                	ret

00000000000008b6 <mkdir>:
.global mkdir
mkdir:
 li a7, SYS_mkdir
     8b6:	48d1                	li	a7,20
 ecall
     8b8:	00000073          	ecall
 ret
     8bc:	8082                	ret

00000000000008be <chdir>:
.global chdir
chdir:
 li a7, SYS_chdir
     8be:	48a5                	li	a7,9
 ecall
     8c0:	00000073          	ecall
 ret
     8c4:	8082                	ret

00000000000008c6 <dup>:
.global dup
dup:
 li a7, SYS_dup
     8c6:	48a9                	li	a7,10
 ecall
     8c8:	00000073          	ecall
 ret
     8cc:	8082                	ret

00000000000008ce <getpid>:
.global getpid
getpid:
 li a7, SYS_getpid
     8ce:	48ad                	li	a7,11
 ecall
     8d0:	00000073          	ecall
 ret
     8d4:	8082                	ret

00000000000008d6 <sbrk>:
.global sbrk
sbrk:
 li a7, SYS_sbrk
     8d6:	48b1                	li	a7,12
 ecall
     8d8:	00000073          	ecall
 ret
     8dc:	8082                	ret

00000000000008de <sleep>:
.global sleep
sleep:
 li a7, SYS_sleep
     8de:	48b5                	li	a7,13
 ecall
     8e0:	00000073          	ecall
 ret
     8e4:	8082                	ret

00000000000008e6 <uptime>:
.global uptime
uptime:
 li a7, SYS_uptime
     8e6:	48b9                	li	a7,14
 ecall
     8e8:	00000073          	ecall
 ret
     8ec:	8082                	ret

00000000000008ee <hello>:
.global hello
hello:
 li a7, SYS_hello
     8ee:	48d9                	li	a7,22
 ecall
     8f0:	00000073          	ecall
 ret
     8f4:	8082                	ret

00000000000008f6 <get_process>:
.global get_process
get_process:
 li a7, SYS_get_process
     8f6:	48dd                	li	a7,23
 ecall
     8f8:	00000073          	ecall
 ret
     8fc:	8082                	ret

00000000000008fe <get_arr>:
.global get_arr
get_arr:
 li a7, SYS_get_arr
     8fe:	48e1                	li	a7,24
 ecall
     900:	00000073          	ecall
 ret
     904:	8082                	ret

0000000000000906 <putc>:

static char digits[] = "0123456789ABCDEF";

static void
putc(int fd, char c)
{
     906:	1101                	addi	sp,sp,-32
     908:	ec06                	sd	ra,24(sp)
     90a:	e822                	sd	s0,16(sp)
     90c:	1000                	addi	s0,sp,32
     90e:	87aa                	mv	a5,a0
     910:	872e                	mv	a4,a1
     912:	fef42623          	sw	a5,-20(s0)
     916:	87ba                	mv	a5,a4
     918:	fef405a3          	sb	a5,-21(s0)
  write(fd, &c, 1);
     91c:	feb40713          	addi	a4,s0,-21
     920:	fec42783          	lw	a5,-20(s0)
     924:	4605                	li	a2,1
     926:	85ba                	mv	a1,a4
     928:	853e                	mv	a0,a5
     92a:	00000097          	auipc	ra,0x0
     92e:	f44080e7          	jalr	-188(ra) # 86e <write>
}
     932:	0001                	nop
     934:	60e2                	ld	ra,24(sp)
     936:	6442                	ld	s0,16(sp)
     938:	6105                	addi	sp,sp,32
     93a:	8082                	ret

000000000000093c <printint>:

static void
printint(int fd, int xx, int base, int sgn)
{
     93c:	7139                	addi	sp,sp,-64
     93e:	fc06                	sd	ra,56(sp)
     940:	f822                	sd	s0,48(sp)
     942:	0080                	addi	s0,sp,64
     944:	87aa                	mv	a5,a0
     946:	8736                	mv	a4,a3
     948:	fcf42623          	sw	a5,-52(s0)
     94c:	87ae                	mv	a5,a1
     94e:	fcf42423          	sw	a5,-56(s0)
     952:	87b2                	mv	a5,a2
     954:	fcf42223          	sw	a5,-60(s0)
     958:	87ba                	mv	a5,a4
     95a:	fcf42023          	sw	a5,-64(s0)
  char buf[16];
  int i, neg;
  uint x;

  neg = 0;
     95e:	fe042423          	sw	zero,-24(s0)
  if(sgn && xx < 0){
     962:	fc042783          	lw	a5,-64(s0)
     966:	2781                	sext.w	a5,a5
     968:	c38d                	beqz	a5,98a <printint+0x4e>
     96a:	fc842783          	lw	a5,-56(s0)
     96e:	2781                	sext.w	a5,a5
     970:	0007dd63          	bgez	a5,98a <printint+0x4e>
    neg = 1;
     974:	4785                	li	a5,1
     976:	fef42423          	sw	a5,-24(s0)
    x = -xx;
     97a:	fc842783          	lw	a5,-56(s0)
     97e:	40f007bb          	negw	a5,a5
     982:	2781                	sext.w	a5,a5
     984:	fef42223          	sw	a5,-28(s0)
     988:	a029                	j	992 <printint+0x56>
  } else {
    x = xx;
     98a:	fc842783          	lw	a5,-56(s0)
     98e:	fef42223          	sw	a5,-28(s0)
  }

  i = 0;
     992:	fe042623          	sw	zero,-20(s0)
  do{
    buf[i++] = digits[x % base];
     996:	fc442783          	lw	a5,-60(s0)
     99a:	fe442703          	lw	a4,-28(s0)
     99e:	02f777bb          	remuw	a5,a4,a5
     9a2:	0007861b          	sext.w	a2,a5
     9a6:	fec42783          	lw	a5,-20(s0)
     9aa:	0017871b          	addiw	a4,a5,1
     9ae:	fee42623          	sw	a4,-20(s0)
     9b2:	00002697          	auipc	a3,0x2
     9b6:	a0e68693          	addi	a3,a3,-1522 # 23c0 <digits>
     9ba:	02061713          	slli	a4,a2,0x20
     9be:	9301                	srli	a4,a4,0x20
     9c0:	9736                	add	a4,a4,a3
     9c2:	00074703          	lbu	a4,0(a4)
     9c6:	17c1                	addi	a5,a5,-16
     9c8:	97a2                	add	a5,a5,s0
     9ca:	fee78023          	sb	a4,-32(a5)
  }while((x /= base) != 0);
     9ce:	fc442783          	lw	a5,-60(s0)
     9d2:	fe442703          	lw	a4,-28(s0)
     9d6:	02f757bb          	divuw	a5,a4,a5
     9da:	fef42223          	sw	a5,-28(s0)
     9de:	fe442783          	lw	a5,-28(s0)
     9e2:	2781                	sext.w	a5,a5
     9e4:	fbcd                	bnez	a5,996 <printint+0x5a>
  if(neg)
     9e6:	fe842783          	lw	a5,-24(s0)
     9ea:	2781                	sext.w	a5,a5
     9ec:	cf85                	beqz	a5,a24 <printint+0xe8>
    buf[i++] = '-';
     9ee:	fec42783          	lw	a5,-20(s0)
     9f2:	0017871b          	addiw	a4,a5,1
     9f6:	fee42623          	sw	a4,-20(s0)
     9fa:	17c1                	addi	a5,a5,-16
     9fc:	97a2                	add	a5,a5,s0
     9fe:	02d00713          	li	a4,45
     a02:	fee78023          	sb	a4,-32(a5)

  while(--i >= 0)
     a06:	a839                	j	a24 <printint+0xe8>
    putc(fd, buf[i]);
     a08:	fec42783          	lw	a5,-20(s0)
     a0c:	17c1                	addi	a5,a5,-16
     a0e:	97a2                	add	a5,a5,s0
     a10:	fe07c703          	lbu	a4,-32(a5)
     a14:	fcc42783          	lw	a5,-52(s0)
     a18:	85ba                	mv	a1,a4
     a1a:	853e                	mv	a0,a5
     a1c:	00000097          	auipc	ra,0x0
     a20:	eea080e7          	jalr	-278(ra) # 906 <putc>
  while(--i >= 0)
     a24:	fec42783          	lw	a5,-20(s0)
     a28:	37fd                	addiw	a5,a5,-1
     a2a:	fef42623          	sw	a5,-20(s0)
     a2e:	fec42783          	lw	a5,-20(s0)
     a32:	2781                	sext.w	a5,a5
     a34:	fc07dae3          	bgez	a5,a08 <printint+0xcc>
}
     a38:	0001                	nop
     a3a:	0001                	nop
     a3c:	70e2                	ld	ra,56(sp)
     a3e:	7442                	ld	s0,48(sp)
     a40:	6121                	addi	sp,sp,64
     a42:	8082                	ret

0000000000000a44 <printptr>:

static void
printptr(int fd, uint64 x) {
     a44:	7179                	addi	sp,sp,-48
     a46:	f406                	sd	ra,40(sp)
     a48:	f022                	sd	s0,32(sp)
     a4a:	1800                	addi	s0,sp,48
     a4c:	87aa                	mv	a5,a0
     a4e:	fcb43823          	sd	a1,-48(s0)
     a52:	fcf42e23          	sw	a5,-36(s0)
  int i;
  putc(fd, '0');
     a56:	fdc42783          	lw	a5,-36(s0)
     a5a:	03000593          	li	a1,48
     a5e:	853e                	mv	a0,a5
     a60:	00000097          	auipc	ra,0x0
     a64:	ea6080e7          	jalr	-346(ra) # 906 <putc>
  putc(fd, 'x');
     a68:	fdc42783          	lw	a5,-36(s0)
     a6c:	07800593          	li	a1,120
     a70:	853e                	mv	a0,a5
     a72:	00000097          	auipc	ra,0x0
     a76:	e94080e7          	jalr	-364(ra) # 906 <putc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
     a7a:	fe042623          	sw	zero,-20(s0)
     a7e:	a82d                	j	ab8 <printptr+0x74>
    putc(fd, digits[x >> (sizeof(uint64) * 8 - 4)]);
     a80:	fd043783          	ld	a5,-48(s0)
     a84:	93f1                	srli	a5,a5,0x3c
     a86:	00002717          	auipc	a4,0x2
     a8a:	93a70713          	addi	a4,a4,-1734 # 23c0 <digits>
     a8e:	97ba                	add	a5,a5,a4
     a90:	0007c703          	lbu	a4,0(a5)
     a94:	fdc42783          	lw	a5,-36(s0)
     a98:	85ba                	mv	a1,a4
     a9a:	853e                	mv	a0,a5
     a9c:	00000097          	auipc	ra,0x0
     aa0:	e6a080e7          	jalr	-406(ra) # 906 <putc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
     aa4:	fec42783          	lw	a5,-20(s0)
     aa8:	2785                	addiw	a5,a5,1
     aaa:	fef42623          	sw	a5,-20(s0)
     aae:	fd043783          	ld	a5,-48(s0)
     ab2:	0792                	slli	a5,a5,0x4
     ab4:	fcf43823          	sd	a5,-48(s0)
     ab8:	fec42783          	lw	a5,-20(s0)
     abc:	873e                	mv	a4,a5
     abe:	47bd                	li	a5,15
     ac0:	fce7f0e3          	bgeu	a5,a4,a80 <printptr+0x3c>
}
     ac4:	0001                	nop
     ac6:	0001                	nop
     ac8:	70a2                	ld	ra,40(sp)
     aca:	7402                	ld	s0,32(sp)
     acc:	6145                	addi	sp,sp,48
     ace:	8082                	ret

0000000000000ad0 <vprintf>:

// Print to the given fd. Only understands %d, %x, %p, %s.
void
vprintf(int fd, const char *fmt, va_list ap)
{
     ad0:	715d                	addi	sp,sp,-80
     ad2:	e486                	sd	ra,72(sp)
     ad4:	e0a2                	sd	s0,64(sp)
     ad6:	0880                	addi	s0,sp,80
     ad8:	87aa                	mv	a5,a0
     ada:	fcb43023          	sd	a1,-64(s0)
     ade:	fac43c23          	sd	a2,-72(s0)
     ae2:	fcf42623          	sw	a5,-52(s0)
  char *s;
  int c, i, state;

  state = 0;
     ae6:	fe042023          	sw	zero,-32(s0)
  for(i = 0; fmt[i]; i++){
     aea:	fe042223          	sw	zero,-28(s0)
     aee:	a42d                	j	d18 <vprintf+0x248>
    c = fmt[i] & 0xff;
     af0:	fe442783          	lw	a5,-28(s0)
     af4:	fc043703          	ld	a4,-64(s0)
     af8:	97ba                	add	a5,a5,a4
     afa:	0007c783          	lbu	a5,0(a5)
     afe:	fcf42e23          	sw	a5,-36(s0)
    if(state == 0){
     b02:	fe042783          	lw	a5,-32(s0)
     b06:	2781                	sext.w	a5,a5
     b08:	eb9d                	bnez	a5,b3e <vprintf+0x6e>
      if(c == '%'){
     b0a:	fdc42783          	lw	a5,-36(s0)
     b0e:	0007871b          	sext.w	a4,a5
     b12:	02500793          	li	a5,37
     b16:	00f71763          	bne	a4,a5,b24 <vprintf+0x54>
        state = '%';
     b1a:	02500793          	li	a5,37
     b1e:	fef42023          	sw	a5,-32(s0)
     b22:	a2f5                	j	d0e <vprintf+0x23e>
      } else {
        putc(fd, c);
     b24:	fdc42783          	lw	a5,-36(s0)
     b28:	0ff7f713          	zext.b	a4,a5
     b2c:	fcc42783          	lw	a5,-52(s0)
     b30:	85ba                	mv	a1,a4
     b32:	853e                	mv	a0,a5
     b34:	00000097          	auipc	ra,0x0
     b38:	dd2080e7          	jalr	-558(ra) # 906 <putc>
     b3c:	aac9                	j	d0e <vprintf+0x23e>
      }
    } else if(state == '%'){
     b3e:	fe042783          	lw	a5,-32(s0)
     b42:	0007871b          	sext.w	a4,a5
     b46:	02500793          	li	a5,37
     b4a:	1cf71263          	bne	a4,a5,d0e <vprintf+0x23e>
      if(c == 'd'){
     b4e:	fdc42783          	lw	a5,-36(s0)
     b52:	0007871b          	sext.w	a4,a5
     b56:	06400793          	li	a5,100
     b5a:	02f71463          	bne	a4,a5,b82 <vprintf+0xb2>
        printint(fd, va_arg(ap, int), 10, 1);
     b5e:	fb843783          	ld	a5,-72(s0)
     b62:	00878713          	addi	a4,a5,8
     b66:	fae43c23          	sd	a4,-72(s0)
     b6a:	4398                	lw	a4,0(a5)
     b6c:	fcc42783          	lw	a5,-52(s0)
     b70:	4685                	li	a3,1
     b72:	4629                	li	a2,10
     b74:	85ba                	mv	a1,a4
     b76:	853e                	mv	a0,a5
     b78:	00000097          	auipc	ra,0x0
     b7c:	dc4080e7          	jalr	-572(ra) # 93c <printint>
     b80:	a269                	j	d0a <vprintf+0x23a>
      } else if(c == 'l') {
     b82:	fdc42783          	lw	a5,-36(s0)
     b86:	0007871b          	sext.w	a4,a5
     b8a:	06c00793          	li	a5,108
     b8e:	02f71663          	bne	a4,a5,bba <vprintf+0xea>
        printint(fd, va_arg(ap, uint64), 10, 0);
     b92:	fb843783          	ld	a5,-72(s0)
     b96:	00878713          	addi	a4,a5,8
     b9a:	fae43c23          	sd	a4,-72(s0)
     b9e:	639c                	ld	a5,0(a5)
     ba0:	0007871b          	sext.w	a4,a5
     ba4:	fcc42783          	lw	a5,-52(s0)
     ba8:	4681                	li	a3,0
     baa:	4629                	li	a2,10
     bac:	85ba                	mv	a1,a4
     bae:	853e                	mv	a0,a5
     bb0:	00000097          	auipc	ra,0x0
     bb4:	d8c080e7          	jalr	-628(ra) # 93c <printint>
     bb8:	aa89                	j	d0a <vprintf+0x23a>
      } else if(c == 'x') {
     bba:	fdc42783          	lw	a5,-36(s0)
     bbe:	0007871b          	sext.w	a4,a5
     bc2:	07800793          	li	a5,120
     bc6:	02f71463          	bne	a4,a5,bee <vprintf+0x11e>
        printint(fd, va_arg(ap, int), 16, 0);
     bca:	fb843783          	ld	a5,-72(s0)
     bce:	00878713          	addi	a4,a5,8
     bd2:	fae43c23          	sd	a4,-72(s0)
     bd6:	4398                	lw	a4,0(a5)
     bd8:	fcc42783          	lw	a5,-52(s0)
     bdc:	4681                	li	a3,0
     bde:	4641                	li	a2,16
     be0:	85ba                	mv	a1,a4
     be2:	853e                	mv	a0,a5
     be4:	00000097          	auipc	ra,0x0
     be8:	d58080e7          	jalr	-680(ra) # 93c <printint>
     bec:	aa39                	j	d0a <vprintf+0x23a>
      } else if(c == 'p') {
     bee:	fdc42783          	lw	a5,-36(s0)
     bf2:	0007871b          	sext.w	a4,a5
     bf6:	07000793          	li	a5,112
     bfa:	02f71263          	bne	a4,a5,c1e <vprintf+0x14e>
        printptr(fd, va_arg(ap, uint64));
     bfe:	fb843783          	ld	a5,-72(s0)
     c02:	00878713          	addi	a4,a5,8
     c06:	fae43c23          	sd	a4,-72(s0)
     c0a:	6398                	ld	a4,0(a5)
     c0c:	fcc42783          	lw	a5,-52(s0)
     c10:	85ba                	mv	a1,a4
     c12:	853e                	mv	a0,a5
     c14:	00000097          	auipc	ra,0x0
     c18:	e30080e7          	jalr	-464(ra) # a44 <printptr>
     c1c:	a0fd                	j	d0a <vprintf+0x23a>
      } else if(c == 's'){
     c1e:	fdc42783          	lw	a5,-36(s0)
     c22:	0007871b          	sext.w	a4,a5
     c26:	07300793          	li	a5,115
     c2a:	04f71c63          	bne	a4,a5,c82 <vprintf+0x1b2>
        s = va_arg(ap, char*);
     c2e:	fb843783          	ld	a5,-72(s0)
     c32:	00878713          	addi	a4,a5,8
     c36:	fae43c23          	sd	a4,-72(s0)
     c3a:	639c                	ld	a5,0(a5)
     c3c:	fef43423          	sd	a5,-24(s0)
        if(s == 0)
     c40:	fe843783          	ld	a5,-24(s0)
     c44:	eb8d                	bnez	a5,c76 <vprintf+0x1a6>
          s = "(null)";
     c46:	00000797          	auipc	a5,0x0
     c4a:	4ea78793          	addi	a5,a5,1258 # 1130 <malloc+0x1b0>
     c4e:	fef43423          	sd	a5,-24(s0)
        while(*s != 0){
     c52:	a015                	j	c76 <vprintf+0x1a6>
          putc(fd, *s);
     c54:	fe843783          	ld	a5,-24(s0)
     c58:	0007c703          	lbu	a4,0(a5)
     c5c:	fcc42783          	lw	a5,-52(s0)
     c60:	85ba                	mv	a1,a4
     c62:	853e                	mv	a0,a5
     c64:	00000097          	auipc	ra,0x0
     c68:	ca2080e7          	jalr	-862(ra) # 906 <putc>
          s++;
     c6c:	fe843783          	ld	a5,-24(s0)
     c70:	0785                	addi	a5,a5,1
     c72:	fef43423          	sd	a5,-24(s0)
        while(*s != 0){
     c76:	fe843783          	ld	a5,-24(s0)
     c7a:	0007c783          	lbu	a5,0(a5)
     c7e:	fbf9                	bnez	a5,c54 <vprintf+0x184>
     c80:	a069                	j	d0a <vprintf+0x23a>
        }
      } else if(c == 'c'){
     c82:	fdc42783          	lw	a5,-36(s0)
     c86:	0007871b          	sext.w	a4,a5
     c8a:	06300793          	li	a5,99
     c8e:	02f71463          	bne	a4,a5,cb6 <vprintf+0x1e6>
        putc(fd, va_arg(ap, uint));
     c92:	fb843783          	ld	a5,-72(s0)
     c96:	00878713          	addi	a4,a5,8
     c9a:	fae43c23          	sd	a4,-72(s0)
     c9e:	439c                	lw	a5,0(a5)
     ca0:	0ff7f713          	zext.b	a4,a5
     ca4:	fcc42783          	lw	a5,-52(s0)
     ca8:	85ba                	mv	a1,a4
     caa:	853e                	mv	a0,a5
     cac:	00000097          	auipc	ra,0x0
     cb0:	c5a080e7          	jalr	-934(ra) # 906 <putc>
     cb4:	a899                	j	d0a <vprintf+0x23a>
      } else if(c == '%'){
     cb6:	fdc42783          	lw	a5,-36(s0)
     cba:	0007871b          	sext.w	a4,a5
     cbe:	02500793          	li	a5,37
     cc2:	00f71f63          	bne	a4,a5,ce0 <vprintf+0x210>
        putc(fd, c);
     cc6:	fdc42783          	lw	a5,-36(s0)
     cca:	0ff7f713          	zext.b	a4,a5
     cce:	fcc42783          	lw	a5,-52(s0)
     cd2:	85ba                	mv	a1,a4
     cd4:	853e                	mv	a0,a5
     cd6:	00000097          	auipc	ra,0x0
     cda:	c30080e7          	jalr	-976(ra) # 906 <putc>
     cde:	a035                	j	d0a <vprintf+0x23a>
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
     ce0:	fcc42783          	lw	a5,-52(s0)
     ce4:	02500593          	li	a1,37
     ce8:	853e                	mv	a0,a5
     cea:	00000097          	auipc	ra,0x0
     cee:	c1c080e7          	jalr	-996(ra) # 906 <putc>
        putc(fd, c);
     cf2:	fdc42783          	lw	a5,-36(s0)
     cf6:	0ff7f713          	zext.b	a4,a5
     cfa:	fcc42783          	lw	a5,-52(s0)
     cfe:	85ba                	mv	a1,a4
     d00:	853e                	mv	a0,a5
     d02:	00000097          	auipc	ra,0x0
     d06:	c04080e7          	jalr	-1020(ra) # 906 <putc>
      }
      state = 0;
     d0a:	fe042023          	sw	zero,-32(s0)
  for(i = 0; fmt[i]; i++){
     d0e:	fe442783          	lw	a5,-28(s0)
     d12:	2785                	addiw	a5,a5,1
     d14:	fef42223          	sw	a5,-28(s0)
     d18:	fe442783          	lw	a5,-28(s0)
     d1c:	fc043703          	ld	a4,-64(s0)
     d20:	97ba                	add	a5,a5,a4
     d22:	0007c783          	lbu	a5,0(a5)
     d26:	dc0795e3          	bnez	a5,af0 <vprintf+0x20>
    }
  }
}
     d2a:	0001                	nop
     d2c:	0001                	nop
     d2e:	60a6                	ld	ra,72(sp)
     d30:	6406                	ld	s0,64(sp)
     d32:	6161                	addi	sp,sp,80
     d34:	8082                	ret

0000000000000d36 <fprintf>:

void
fprintf(int fd, const char *fmt, ...)
{
     d36:	7159                	addi	sp,sp,-112
     d38:	fc06                	sd	ra,56(sp)
     d3a:	f822                	sd	s0,48(sp)
     d3c:	0080                	addi	s0,sp,64
     d3e:	fcb43823          	sd	a1,-48(s0)
     d42:	e010                	sd	a2,0(s0)
     d44:	e414                	sd	a3,8(s0)
     d46:	e818                	sd	a4,16(s0)
     d48:	ec1c                	sd	a5,24(s0)
     d4a:	03043023          	sd	a6,32(s0)
     d4e:	03143423          	sd	a7,40(s0)
     d52:	87aa                	mv	a5,a0
     d54:	fcf42e23          	sw	a5,-36(s0)
  va_list ap;

  va_start(ap, fmt);
     d58:	03040793          	addi	a5,s0,48
     d5c:	fcf43423          	sd	a5,-56(s0)
     d60:	fc843783          	ld	a5,-56(s0)
     d64:	fd078793          	addi	a5,a5,-48
     d68:	fef43423          	sd	a5,-24(s0)
  vprintf(fd, fmt, ap);
     d6c:	fe843703          	ld	a4,-24(s0)
     d70:	fdc42783          	lw	a5,-36(s0)
     d74:	863a                	mv	a2,a4
     d76:	fd043583          	ld	a1,-48(s0)
     d7a:	853e                	mv	a0,a5
     d7c:	00000097          	auipc	ra,0x0
     d80:	d54080e7          	jalr	-684(ra) # ad0 <vprintf>
}
     d84:	0001                	nop
     d86:	70e2                	ld	ra,56(sp)
     d88:	7442                	ld	s0,48(sp)
     d8a:	6165                	addi	sp,sp,112
     d8c:	8082                	ret

0000000000000d8e <printf>:

void
printf(const char *fmt, ...)
{
     d8e:	7159                	addi	sp,sp,-112
     d90:	f406                	sd	ra,40(sp)
     d92:	f022                	sd	s0,32(sp)
     d94:	1800                	addi	s0,sp,48
     d96:	fca43c23          	sd	a0,-40(s0)
     d9a:	e40c                	sd	a1,8(s0)
     d9c:	e810                	sd	a2,16(s0)
     d9e:	ec14                	sd	a3,24(s0)
     da0:	f018                	sd	a4,32(s0)
     da2:	f41c                	sd	a5,40(s0)
     da4:	03043823          	sd	a6,48(s0)
     da8:	03143c23          	sd	a7,56(s0)
  va_list ap;

  va_start(ap, fmt);
     dac:	04040793          	addi	a5,s0,64
     db0:	fcf43823          	sd	a5,-48(s0)
     db4:	fd043783          	ld	a5,-48(s0)
     db8:	fc878793          	addi	a5,a5,-56
     dbc:	fef43423          	sd	a5,-24(s0)
  vprintf(1, fmt, ap);
     dc0:	fe843783          	ld	a5,-24(s0)
     dc4:	863e                	mv	a2,a5
     dc6:	fd843583          	ld	a1,-40(s0)
     dca:	4505                	li	a0,1
     dcc:	00000097          	auipc	ra,0x0
     dd0:	d04080e7          	jalr	-764(ra) # ad0 <vprintf>
}
     dd4:	0001                	nop
     dd6:	70a2                	ld	ra,40(sp)
     dd8:	7402                	ld	s0,32(sp)
     dda:	6165                	addi	sp,sp,112
     ddc:	8082                	ret

0000000000000dde <free>:
static Header base;
static Header *freep;

void
free(void *ap)
{
     dde:	7179                	addi	sp,sp,-48
     de0:	f422                	sd	s0,40(sp)
     de2:	1800                	addi	s0,sp,48
     de4:	fca43c23          	sd	a0,-40(s0)
  Header *bp, *p;

  bp = (Header*)ap - 1;
     de8:	fd843783          	ld	a5,-40(s0)
     dec:	17c1                	addi	a5,a5,-16
     dee:	fef43023          	sd	a5,-32(s0)
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
     df2:	00001797          	auipc	a5,0x1
     df6:	60e78793          	addi	a5,a5,1550 # 2400 <freep>
     dfa:	639c                	ld	a5,0(a5)
     dfc:	fef43423          	sd	a5,-24(s0)
     e00:	a815                	j	e34 <free+0x56>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
     e02:	fe843783          	ld	a5,-24(s0)
     e06:	639c                	ld	a5,0(a5)
     e08:	fe843703          	ld	a4,-24(s0)
     e0c:	00f76f63          	bltu	a4,a5,e2a <free+0x4c>
     e10:	fe043703          	ld	a4,-32(s0)
     e14:	fe843783          	ld	a5,-24(s0)
     e18:	02e7eb63          	bltu	a5,a4,e4e <free+0x70>
     e1c:	fe843783          	ld	a5,-24(s0)
     e20:	639c                	ld	a5,0(a5)
     e22:	fe043703          	ld	a4,-32(s0)
     e26:	02f76463          	bltu	a4,a5,e4e <free+0x70>
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
     e2a:	fe843783          	ld	a5,-24(s0)
     e2e:	639c                	ld	a5,0(a5)
     e30:	fef43423          	sd	a5,-24(s0)
     e34:	fe043703          	ld	a4,-32(s0)
     e38:	fe843783          	ld	a5,-24(s0)
     e3c:	fce7f3e3          	bgeu	a5,a4,e02 <free+0x24>
     e40:	fe843783          	ld	a5,-24(s0)
     e44:	639c                	ld	a5,0(a5)
     e46:	fe043703          	ld	a4,-32(s0)
     e4a:	faf77ce3          	bgeu	a4,a5,e02 <free+0x24>
      break;
  if(bp + bp->s.size == p->s.ptr){
     e4e:	fe043783          	ld	a5,-32(s0)
     e52:	479c                	lw	a5,8(a5)
     e54:	1782                	slli	a5,a5,0x20
     e56:	9381                	srli	a5,a5,0x20
     e58:	0792                	slli	a5,a5,0x4
     e5a:	fe043703          	ld	a4,-32(s0)
     e5e:	973e                	add	a4,a4,a5
     e60:	fe843783          	ld	a5,-24(s0)
     e64:	639c                	ld	a5,0(a5)
     e66:	02f71763          	bne	a4,a5,e94 <free+0xb6>
    bp->s.size += p->s.ptr->s.size;
     e6a:	fe043783          	ld	a5,-32(s0)
     e6e:	4798                	lw	a4,8(a5)
     e70:	fe843783          	ld	a5,-24(s0)
     e74:	639c                	ld	a5,0(a5)
     e76:	479c                	lw	a5,8(a5)
     e78:	9fb9                	addw	a5,a5,a4
     e7a:	0007871b          	sext.w	a4,a5
     e7e:	fe043783          	ld	a5,-32(s0)
     e82:	c798                	sw	a4,8(a5)
    bp->s.ptr = p->s.ptr->s.ptr;
     e84:	fe843783          	ld	a5,-24(s0)
     e88:	639c                	ld	a5,0(a5)
     e8a:	6398                	ld	a4,0(a5)
     e8c:	fe043783          	ld	a5,-32(s0)
     e90:	e398                	sd	a4,0(a5)
     e92:	a039                	j	ea0 <free+0xc2>
  } else
    bp->s.ptr = p->s.ptr;
     e94:	fe843783          	ld	a5,-24(s0)
     e98:	6398                	ld	a4,0(a5)
     e9a:	fe043783          	ld	a5,-32(s0)
     e9e:	e398                	sd	a4,0(a5)
  if(p + p->s.size == bp){
     ea0:	fe843783          	ld	a5,-24(s0)
     ea4:	479c                	lw	a5,8(a5)
     ea6:	1782                	slli	a5,a5,0x20
     ea8:	9381                	srli	a5,a5,0x20
     eaa:	0792                	slli	a5,a5,0x4
     eac:	fe843703          	ld	a4,-24(s0)
     eb0:	97ba                	add	a5,a5,a4
     eb2:	fe043703          	ld	a4,-32(s0)
     eb6:	02f71563          	bne	a4,a5,ee0 <free+0x102>
    p->s.size += bp->s.size;
     eba:	fe843783          	ld	a5,-24(s0)
     ebe:	4798                	lw	a4,8(a5)
     ec0:	fe043783          	ld	a5,-32(s0)
     ec4:	479c                	lw	a5,8(a5)
     ec6:	9fb9                	addw	a5,a5,a4
     ec8:	0007871b          	sext.w	a4,a5
     ecc:	fe843783          	ld	a5,-24(s0)
     ed0:	c798                	sw	a4,8(a5)
    p->s.ptr = bp->s.ptr;
     ed2:	fe043783          	ld	a5,-32(s0)
     ed6:	6398                	ld	a4,0(a5)
     ed8:	fe843783          	ld	a5,-24(s0)
     edc:	e398                	sd	a4,0(a5)
     ede:	a031                	j	eea <free+0x10c>
  } else
    p->s.ptr = bp;
     ee0:	fe843783          	ld	a5,-24(s0)
     ee4:	fe043703          	ld	a4,-32(s0)
     ee8:	e398                	sd	a4,0(a5)
  freep = p;
     eea:	00001797          	auipc	a5,0x1
     eee:	51678793          	addi	a5,a5,1302 # 2400 <freep>
     ef2:	fe843703          	ld	a4,-24(s0)
     ef6:	e398                	sd	a4,0(a5)
}
     ef8:	0001                	nop
     efa:	7422                	ld	s0,40(sp)
     efc:	6145                	addi	sp,sp,48
     efe:	8082                	ret

0000000000000f00 <morecore>:

static Header*
morecore(uint nu)
{
     f00:	7179                	addi	sp,sp,-48
     f02:	f406                	sd	ra,40(sp)
     f04:	f022                	sd	s0,32(sp)
     f06:	1800                	addi	s0,sp,48
     f08:	87aa                	mv	a5,a0
     f0a:	fcf42e23          	sw	a5,-36(s0)
  char *p;
  Header *hp;

  if(nu < 4096)
     f0e:	fdc42783          	lw	a5,-36(s0)
     f12:	0007871b          	sext.w	a4,a5
     f16:	6785                	lui	a5,0x1
     f18:	00f77563          	bgeu	a4,a5,f22 <morecore+0x22>
    nu = 4096;
     f1c:	6785                	lui	a5,0x1
     f1e:	fcf42e23          	sw	a5,-36(s0)
  p = sbrk(nu * sizeof(Header));
     f22:	fdc42783          	lw	a5,-36(s0)
     f26:	0047979b          	slliw	a5,a5,0x4
     f2a:	2781                	sext.w	a5,a5
     f2c:	2781                	sext.w	a5,a5
     f2e:	853e                	mv	a0,a5
     f30:	00000097          	auipc	ra,0x0
     f34:	9a6080e7          	jalr	-1626(ra) # 8d6 <sbrk>
     f38:	fea43423          	sd	a0,-24(s0)
  if(p == (char*)-1)
     f3c:	fe843703          	ld	a4,-24(s0)
     f40:	57fd                	li	a5,-1
     f42:	00f71463          	bne	a4,a5,f4a <morecore+0x4a>
    return 0;
     f46:	4781                	li	a5,0
     f48:	a03d                	j	f76 <morecore+0x76>
  hp = (Header*)p;
     f4a:	fe843783          	ld	a5,-24(s0)
     f4e:	fef43023          	sd	a5,-32(s0)
  hp->s.size = nu;
     f52:	fe043783          	ld	a5,-32(s0)
     f56:	fdc42703          	lw	a4,-36(s0)
     f5a:	c798                	sw	a4,8(a5)
  free((void*)(hp + 1));
     f5c:	fe043783          	ld	a5,-32(s0)
     f60:	07c1                	addi	a5,a5,16 # 1010 <malloc+0x90>
     f62:	853e                	mv	a0,a5
     f64:	00000097          	auipc	ra,0x0
     f68:	e7a080e7          	jalr	-390(ra) # dde <free>
  return freep;
     f6c:	00001797          	auipc	a5,0x1
     f70:	49478793          	addi	a5,a5,1172 # 2400 <freep>
     f74:	639c                	ld	a5,0(a5)
}
     f76:	853e                	mv	a0,a5
     f78:	70a2                	ld	ra,40(sp)
     f7a:	7402                	ld	s0,32(sp)
     f7c:	6145                	addi	sp,sp,48
     f7e:	8082                	ret

0000000000000f80 <malloc>:

void*
malloc(uint nbytes)
{
     f80:	7139                	addi	sp,sp,-64
     f82:	fc06                	sd	ra,56(sp)
     f84:	f822                	sd	s0,48(sp)
     f86:	0080                	addi	s0,sp,64
     f88:	87aa                	mv	a5,a0
     f8a:	fcf42623          	sw	a5,-52(s0)
  Header *p, *prevp;
  uint nunits;

  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
     f8e:	fcc46783          	lwu	a5,-52(s0)
     f92:	07bd                	addi	a5,a5,15
     f94:	8391                	srli	a5,a5,0x4
     f96:	2781                	sext.w	a5,a5
     f98:	2785                	addiw	a5,a5,1
     f9a:	fcf42e23          	sw	a5,-36(s0)
  if((prevp = freep) == 0){
     f9e:	00001797          	auipc	a5,0x1
     fa2:	46278793          	addi	a5,a5,1122 # 2400 <freep>
     fa6:	639c                	ld	a5,0(a5)
     fa8:	fef43023          	sd	a5,-32(s0)
     fac:	fe043783          	ld	a5,-32(s0)
     fb0:	ef95                	bnez	a5,fec <malloc+0x6c>
    base.s.ptr = freep = prevp = &base;
     fb2:	00001797          	auipc	a5,0x1
     fb6:	43e78793          	addi	a5,a5,1086 # 23f0 <base>
     fba:	fef43023          	sd	a5,-32(s0)
     fbe:	00001797          	auipc	a5,0x1
     fc2:	44278793          	addi	a5,a5,1090 # 2400 <freep>
     fc6:	fe043703          	ld	a4,-32(s0)
     fca:	e398                	sd	a4,0(a5)
     fcc:	00001797          	auipc	a5,0x1
     fd0:	43478793          	addi	a5,a5,1076 # 2400 <freep>
     fd4:	6398                	ld	a4,0(a5)
     fd6:	00001797          	auipc	a5,0x1
     fda:	41a78793          	addi	a5,a5,1050 # 23f0 <base>
     fde:	e398                	sd	a4,0(a5)
    base.s.size = 0;
     fe0:	00001797          	auipc	a5,0x1
     fe4:	41078793          	addi	a5,a5,1040 # 23f0 <base>
     fe8:	0007a423          	sw	zero,8(a5)
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
     fec:	fe043783          	ld	a5,-32(s0)
     ff0:	639c                	ld	a5,0(a5)
     ff2:	fef43423          	sd	a5,-24(s0)
    if(p->s.size >= nunits){
     ff6:	fe843783          	ld	a5,-24(s0)
     ffa:	4798                	lw	a4,8(a5)
     ffc:	fdc42783          	lw	a5,-36(s0)
    1000:	2781                	sext.w	a5,a5
    1002:	06f76763          	bltu	a4,a5,1070 <malloc+0xf0>
      if(p->s.size == nunits)
    1006:	fe843783          	ld	a5,-24(s0)
    100a:	4798                	lw	a4,8(a5)
    100c:	fdc42783          	lw	a5,-36(s0)
    1010:	2781                	sext.w	a5,a5
    1012:	00e79963          	bne	a5,a4,1024 <malloc+0xa4>
        prevp->s.ptr = p->s.ptr;
    1016:	fe843783          	ld	a5,-24(s0)
    101a:	6398                	ld	a4,0(a5)
    101c:	fe043783          	ld	a5,-32(s0)
    1020:	e398                	sd	a4,0(a5)
    1022:	a825                	j	105a <malloc+0xda>
      else {
        p->s.size -= nunits;
    1024:	fe843783          	ld	a5,-24(s0)
    1028:	479c                	lw	a5,8(a5)
    102a:	fdc42703          	lw	a4,-36(s0)
    102e:	9f99                	subw	a5,a5,a4
    1030:	0007871b          	sext.w	a4,a5
    1034:	fe843783          	ld	a5,-24(s0)
    1038:	c798                	sw	a4,8(a5)
        p += p->s.size;
    103a:	fe843783          	ld	a5,-24(s0)
    103e:	479c                	lw	a5,8(a5)
    1040:	1782                	slli	a5,a5,0x20
    1042:	9381                	srli	a5,a5,0x20
    1044:	0792                	slli	a5,a5,0x4
    1046:	fe843703          	ld	a4,-24(s0)
    104a:	97ba                	add	a5,a5,a4
    104c:	fef43423          	sd	a5,-24(s0)
        p->s.size = nunits;
    1050:	fe843783          	ld	a5,-24(s0)
    1054:	fdc42703          	lw	a4,-36(s0)
    1058:	c798                	sw	a4,8(a5)
      }
      freep = prevp;
    105a:	00001797          	auipc	a5,0x1
    105e:	3a678793          	addi	a5,a5,934 # 2400 <freep>
    1062:	fe043703          	ld	a4,-32(s0)
    1066:	e398                	sd	a4,0(a5)
      return (void*)(p + 1);
    1068:	fe843783          	ld	a5,-24(s0)
    106c:	07c1                	addi	a5,a5,16
    106e:	a091                	j	10b2 <malloc+0x132>
    }
    if(p == freep)
    1070:	00001797          	auipc	a5,0x1
    1074:	39078793          	addi	a5,a5,912 # 2400 <freep>
    1078:	639c                	ld	a5,0(a5)
    107a:	fe843703          	ld	a4,-24(s0)
    107e:	02f71063          	bne	a4,a5,109e <malloc+0x11e>
      if((p = morecore(nunits)) == 0)
    1082:	fdc42783          	lw	a5,-36(s0)
    1086:	853e                	mv	a0,a5
    1088:	00000097          	auipc	ra,0x0
    108c:	e78080e7          	jalr	-392(ra) # f00 <morecore>
    1090:	fea43423          	sd	a0,-24(s0)
    1094:	fe843783          	ld	a5,-24(s0)
    1098:	e399                	bnez	a5,109e <malloc+0x11e>
        return 0;
    109a:	4781                	li	a5,0
    109c:	a819                	j	10b2 <malloc+0x132>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    109e:	fe843783          	ld	a5,-24(s0)
    10a2:	fef43023          	sd	a5,-32(s0)
    10a6:	fe843783          	ld	a5,-24(s0)
    10aa:	639c                	ld	a5,0(a5)
    10ac:	fef43423          	sd	a5,-24(s0)
    if(p->s.size >= nunits){
    10b0:	b799                	j	ff6 <malloc+0x76>
  }
}
    10b2:	853e                	mv	a0,a5
    10b4:	70e2                	ld	ra,56(sp)
    10b6:	7442                	ld	s0,48(sp)
    10b8:	6121                	addi	sp,sp,64
    10ba:	8082                	ret
